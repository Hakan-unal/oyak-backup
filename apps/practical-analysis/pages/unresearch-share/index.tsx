...
import { DEFAULT_UNRESEARCH_SHARE } from 'prac-analysis/sections/unresearch-share/constants';

const UnresearchShare = () => {
  const {
    query: { symbol },
  } = useRouter();
  ...
  const { data: assets } = useQuery(QUERY_KEYS.GET_INSTRUMENT_ANALYZE, () =>
    getInstrumentAnalyzeHandler({ includesResearchScope: false }),
  );
  ...
  useEffect(() => {
    if (symbol) {
      ...
      setAssetId(DEFAULT_UNRESEARCH_SHARE);
    }
  }, [ symbol, assets ]);
...
export default UnresearchShare;
