...
const schema = object().shape({
  ...
  communication: string()
    .required("general_form_requiredField_text")
    .oneOf([ "yes" ], "insuranceRequest_form_information4_errorMessage"),
  kvkk: boolean()
    .required("general_form_requiredField_text")
...
const BesForm = () => {
  ...
  const mutation = useMutation({
    mutationFn: (data: any) =>
      InternalApi.post(RemoteApiPaths.Bes, data).then(() => {
        push(paths.success);
      }),
    onError() {
      push("/insurance/fail");
    ...
    <Layout title="insuranceRequest_form_informations_title">
      ...
      >
        <Flex direction="column" gap="4" px="4">
          ...
          />
          <TranslatedText
            label="insuranceRequest_form_bes_communicationPermission_text"
            mb="0"
            variant="rBody2"
          />
          <RadioGroup
