...
async function complianceTestRoute(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!user) {
    ...
    try {
      ...
      const response = await RemoteApi(token?.accessToken)
        .post(Endpoints.Account.ComplianceTest, params)
        .then((r) => r.data);
...
export default withIronSessionApiRoute(complianceTestRoute, sessionOptions);
